...
- [x] TODO-048: **CRITICAL** - Buy/Sell buttons fixed with vanilla JS fetch() approach. The HTMX event chain (htmx:beforeRequest) was unreliable for dynamically-added forms in the orderbook. **FIX**: Replaced HTMX-based aggress forms with vanilla JavaScript fetch() calls. New `executeAggress()` function handles: click size, spinner, aggress lock, POST via fetch(), toast parsing, lock release. `attachAggressHandlers()` attaches click handlers to Buy/Sell buttons on page load and after each WebSocket DOM update. **VERIFIED**: Created verify_aggress.py script that tests end-to-end: login → place order → aggress → verify trade. Server logs show requests arriving correctly. All 129 pytest tests pass. Code deployed to GitHub for Render auto-deploy.
...
- [x] TODO-049: **WebSocket was broken in production** - Human investigation found `requirements.txt` had `uvicorn>=0.24.0` but needed `uvicorn[standard]>=0.24.0` for WebSocket support. Render logs showed "No supported WebSocket library detected" and `/ws/market/...` returned 404. **FIX DEPLOYED by human**: Changed to `uvicorn[standard]`. Screenshots confirmed the issue was real and occurred BEFORE the fix was deployed. Lesson added to LESSONS.md.
